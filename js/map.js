...
(function () {
  ...
  // -----------------------------------------------------------------------------
  // Пункт ТЗ 4.1.::
  // Приблизительный адрес квартиры указывается перетаскиванием специальной метки по
  // карте Токио. При перемещении изменяется значение соответствующего поля ввода.
  // Содержимое поле адреса не может быть пустым: сразу после загрузки страницы и после
  // сброса формы, значение должно соответствовать координатам метки. В неактивном режиме
  // страницы метка круглая, поэтому за исходное значение поля адреса следует брать
  // середину метки. При «перетасквании» метки, в поле адреса подставляются координаты,
  // указывающие на острый конец метки.
  // -------------------------------------------------------------------------------
  ...
  var setInactiveStatus = function () {
    ...
    setInactiveAddress(xPinMain, yPinMain); // установим в поле адреса первоначальные координаты, центр круга - см.[1]
  ...
  pinMain.addEventListener('mousedown', function (evt) {
    ...
    // в момент нажатия на метку координаты считаются уже по краю пика, см. ТЗ пункт 4.1 или [1] выше
    var startPinCoords = {
